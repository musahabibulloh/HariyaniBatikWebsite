...
export default function EditActivity({ params }: { params: Promise<{ id: string }> }) {
    ...
    async function loadActivity() {
      try {
        ...
        if (activity) {
          ...
          setCurrentImageUrl(activity.image_url);
        }
      } catch (err) {
  ...
  if (initialLoading) {
    return <div className="py-20 text-center text-gray-500">Memuat data kegiatan...</div>;
  }
  ...
  return (
    <div className="py-10 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-1">Ubah artikel atau liputan kegiatan.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-lg shadow border border-gray-200 space-y-6">
        ...
        <div>
          <h2 className="text-sm font-medium text-gray-700 mb-2">Gambar Kegiatan</h2>
          <div className="flex gap-6 items-start">
            {currentImageUrl && (
              <div className="w-32 h-32 rounded bg-gray-100 overflow-hidden shrink-0 border border-gray-200">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={currentImageUrl} alt="Current Activity" className="w-full h-full object-cover" />
              </div>
            )}
            <div className="flex-1">
              <label htmlFor="image" className="block text-sm font-medium text-gray-700 mb-1">Ubah Gambar</label>
